...
if(!class_exists('Esoftcreator_DB_Helper')):
	require_once( 'class-esoftcreator-db-query-helper.php');	
	class Esoftcreator_DB_Helper extends Esoftcreator_DB_Query_Helper{
		...
		public function get_sales_reports_overview($start_date, $end_date){ 
			global $wpdb;
		  $posts_table = $wpdb->prefix."posts";
		  $postmeta_table = $wpdb->prefix."postmeta";
		  $woocommerce_order_itemmeta = $wpdb->prefix."woocommerce_order_itemmeta";
		 	...
		 	if(!empty($results)){
			 	foreach ($results as $key => $value) {
			 		if($value['order_status'] == $sub_results_1[$key]['order_status']){
			 			...
			 			$results['order_status'][$order_status_slug] = array_merge($results[$key],$sub_results_1[$key]);
			 			unset($results[$key]);
			 			//save current row value
			 			$temp_result = $results['order_status'][$order_status_slug];

			 			$order_status[$order_status_slug] = $temp_result['total_orders'];
			 			$total_sale += $temp_result['order_total'];
			 			$net_sale+=$temp_result['line_total'];
			 			$total_orders+=$temp_result['total_orders'];
			 			$refund_order+=$temp_result['refund_order'];
			 			$refund_order_value+=$temp_result['refund_amount'];
			 			$discount_amount+=$temp_result['discount_amount'];
			 			$total_tax+=$temp_result['order_tax']+$temp_result['order_shipping_tax'];
			 			$order_tax+=$temp_result['order_tax'];
			 			$shipping_tax+=$temp_result['order_shipping_tax'];
			 			$shipping+=$temp_result['shipping'];
			 		}
			 	}		 	
		 	}	
		 	if($total_sale>0){
		...
		public function get_cart_abandoned($start_date, $end_date){
			$abandoned_interval = date("Y-m-d H:i", (current_time( 'timestamp')- 1200));
			global $wpdb;
		  $table = $wpdb->prefix."esc_user_behavior";
			$sql =  $wpdb->prepare("SELECT ub.* from ".$table." as ub 
				WHERE ub.purchase != 1 AND ub.created_at < '%s' AND DATE(ub.created_at) >= '%s' AND DATE(ub.created_at) <= '%s'", $abandoned_interval, $start_date, $end_date);
